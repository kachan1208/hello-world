...
## Purpose 
Develop a SAML Assertion Generator Service that will generate a SAML assertion on Ionic Cloud from the 3rd-party infrastucture. SAML Assertion Generator Service has own Master Service Key that is generated after its deployment and stored in Hashicorp Vault in Virgil Infrastructure.
...
- Virgil Cloud
- Ionic Cloud
- End-users (Developers) and their Devices 
- Hashicorp Vault - encrypted keys storage
- Virgil-Ionic High Level SDK

#### How it works

#### How it works from a Developer Side
...
- Sends a request with SAML to specified Ionic Enrollment server to Enroll (register) a device 

##### How it works from Virgil Side
...
- Virgil Infrastructure receives the RSA Key, encrypts it with the Server Master Key and stores it into own Cassandra Database

## Potential SAML Service API
SAML assertion generator service have only one API Endpoint: it receives a bunch of data about user and generates SAML assertion for specified Enrollment service in Ionic Cloud

### Prerequisites 
- Virgil JWT. The End-user must have a JWT to comunicate with Virgil Cloud. The JWT is generated by Application Service. 

### POST /saml Endpoint 
The Endpoint generates a SAML assertion to enroll devices.
